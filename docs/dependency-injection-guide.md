...
- **프로덕션 코드**: `@Autowired private var service` 로 캡슐화 권장
- **테스트 가능 코드**: 테스트가 필요한 Core에서는 `@Autowired var service` 로 internal/public 선언 필요
...
CoreFlow에서는 테스트 시 ServiceLocator에 의존성을 등록하지 않고, `@Autowired`로 선언된 프로퍼티에 직접 mock/stub을 주입할 수 있습니다.
...
@Test func example() async throws {
    // Given
    let sut = LoginCore(initialState: .init())
    let testUser = User(name: "Test user")
    sut.service = StubLoginService(user: testUser)  // 직접 mock 주입
    sut.enableTestMode()
...
CoreFlow의 테스트 헬퍼 메서드입니다. 테스트 시 비동기 액션의 완료를 대기할 수 있습니다.

```swift
sut.enableTestMode()
sut.send(.someAction)
try await sut.exhaust()  // 모든 비동기 작업 완료 대기
```

### 주의 사항

- 테스트 시 `@Autowired` 프로퍼티는 **internal 또는 public** 접근 제어자를 사용해야 합니다.
- 테스트가 아닌 코드에서는 `private`으로 선언하여 캡슐화를 유지하세요.
- `enableTestMode()`를 호출해야 `exhaust()` 메서드를 사용할 수 있습니다.
